...
function getPoint() {
	...
	if(close == 1){
		...
		$('#submit_buy').val('提交订单');
		$('#submit_buy').html('提交订单');
	}
	var inputnametype = '';
	$('#inputsname').html("");
	var inputname = $('#tid').attr('inputname');
	if(inputname=='hide'){
		$('#inputsname').append('<tr><td style="text-align: right;"><span class="slivergrey"></span></td><td><input type="hidden" name="inputvalue" id="inputvalue" value="'+$.cookie('mysid')+'"/></td></tr>');
		$('#orderinfo').hide();
	}else{
		...
		$('#inputsname').append('<tr><td style="text-align: right;"><span class="slivergrey" id="inputname">'+inputname+'：</span></td><td><input type="text" name="inputvalue" id="inputvalue" value="'+($_GET['qq']?$_GET['qq']:'')+'" class="form-control" required onblur="checkInput()"/></td></tr>');
	}
	...
	if(inputsname!=''){
		$.each(inputsname.split('|'), function(i, value) {
			...
			if(value.indexOf('{')>0 && value.indexOf('}')>0){
				...
				$('#inputsname').append('<tr><td style="text-align: right;"><span class="slivergrey" id="inputname'+(i+2)+'">'+selectname+'：</span></td><td><select name="inputvalue'+(i+2)+'" id="inputvalue'+(i+2)+'" class="form-control">'+addstr+'</select></td></tr>');
			}else{
			if(value=='说说ID'||value=='说说ＩＤ'||inputsnametype=='ssid')
				var addstr='<div class="btn-click" onclick="get_shuoshuo(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">自动获取</div>';
			else if(value=='日志ID'||value=='日志ＩＤ'||inputsnametype=='rzid')
				var addstr='<div class="btn-click" onclick="get_rizhi(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">自动获取</div>';
			else if(value=='作品ID'||value=='作品ＩＤ'||inputsnametype=='zpid')
				var addstr='<div class="btn-click" onclick="getshareid2(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">自动获取</div>';
			else if(value=='收货地址'||value=='收货人地址'||inputsnametype=='address')
				var addstr='<div class="btn-click" onclick="getCity(\'inputvalue'+(i+2)+'\')">点此选择</div>';
			else
				var addstr='';
			$('#inputsname').append('<tr><td style="text-align: right;"><span class="slivergrey" id="inputname'+(i+2)+'" gettype="'+inputsnametype+'">'+value+'：</span></td><td><input type="text" name="inputvalue'+(i+2)+'" id="inputvalue'+(i+2)+'" value="" class="form-control" required/></td><td>'+addstr+'</td></tr>');
			}
		});
	}
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if($('#show_shuoshuo').length > 0){
					$('#show_shuoshuo').html('<td><div class="btn-click" title="上一页" onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')" style="float: right;">上一页</div></td><td><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select></td><td><div class="btn-click" title="下一页" onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')">下一页</div></td>');
				}else{
					$('#inputsname').append('<tr id="show_shuoshuo"><td><div class="btn-click" title="上一页" onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')" style="float: right;">上一页</div></td><td><select id="shuoid" class="form-control" onchange="set_shuoshuo(\''+id+'\');">'+addstr+'</select></td><td><div class="btn-click" title="下一页" onclick="get_shuoshuo(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')">下一页</div></td></tr>');
				}
				set_shuoshuo(id);
			}else{
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if($('#show_rizhi').length > 0){
					$('#show_rizhi').html('<td><div class="btn-click" title="上一页" onclick="get_rizhi(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')" style="float: right;">上一页</div></td><td><select id="shuoid" class="form-control" onchange="set_rizhi(\''+id+'\');">'+addstr+'</select></td><td><div class="btn-click" title="下一页" onclick="get_rizhi(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')">下一页</div></td>');
				}else{
					$('#inputsname').append('<tr id="show_rizhi"><td><div class="btn-click" title="上一页" onclick="get_rizhi(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+lastpage+')" style="float: right;">上一页</div></td><td><select id="shuoid" class="form-control" onchange="set_rizhi(\''+id+'\');">'+addstr+'</select></td><td><div class="btn-click" title="下一页" onclick="get_rizhi(\''+id+'\',$(\'#inputvalue\').val(),'+km+','+nextpage+')">下一页</div></td></tr>');
				}
				set_rizhi(id);
			}else{
				layer.alert(data.msg);
...
$(document).ready(function(){
	$("#submit_buy").click(function(){
		...
		if($("#paylist li a.selected").length==0){
			layer.msg('请选择支付方式');
			return false;
		}
		var paytype = $("#paylist li a.selected").attr('id');
		var ii = layer.load(2, {shade:[0.1,'#fff']});
		$.ajax({
			type : "POST",
			url : "ajax.php?act=pay",
			data : {tid:tid,inputvalue:$("#inputvalue").val(),inputvalue2:$("#inputvalue2").val(),inputvalue3:$("#inputvalue3").val(),inputvalue4:$("#inputvalue4").val(),inputvalue5:$("#inputvalue5").val(),num:$("#num").val(),hashsalt:hashsalt,paytype:paytype},
			dataType : 'json',
			success : function(data) {
				layer.close(ii);
				if(data.code == 0){
