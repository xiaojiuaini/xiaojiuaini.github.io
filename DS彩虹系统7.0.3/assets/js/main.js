    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.code == 0) {
                ...
                if (data.complain) {
                    item += '<tr><td class="info orderTitle">订单操作</td><td class="orderContent"><a href="./user/workorder.php?my=add&orderid=' + id + '&skey=' + skey + '" target="_blank" onclick="return checklogin(' + data.islogin + ')" class="btn btn-block btn-default">提交工单</a></td><td class="orderContent"><a href="#" onclick="return orderRefund(' + data.islogin + ',' + id + ')" class="btn btn-block btn-danger">申请退款</a></td></tr>';
                }
...
function orderRefund(islogin, id) {
    if (!islogin) {
        var confirmobj = layer.confirm('待处理或异常订单可以申请退款，退款之后资金会退到用户余额，所以前请先登录网站！', {
            btn: ['登录', '注册', '取消']
        }, function () {
            window.location.href = './user/login.php';
        }, function () {
    ...
    var confirmobj = layer.confirm('待处理或异常订单可以申请退款，退款之后资金会退到用户余额，是否确认退款？', {
        btn: ['确认退款', '取消']
    }, function () {
        $.ajax({
            type: "POST",
            url: "/user/ajax_user.php?act=refund",
            data: {'id': id},
            dataType: 'json',
            success: function (data) {
                if (data.code == 0) {
