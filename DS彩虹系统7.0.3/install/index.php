	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=mysqli_connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(mysqli_query($con, "select * from ".$dbconfig['dbqz']."_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过彩虹其它管理系统</div>
...
?>';
		if(!$con=mysqli_connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			echo '<div class="alert alert-warning">MySQL数据库版本太低，需要MySQL 5.6或以上版本推荐7.0！</div>';
			...
			if(mysqli_query($con, "select * from ".$db_qz."_config where 1")==FALSE)
				...
				<div class="list-group-item">
					<a href="?do=6" class="btn btn-block btn-info">跳过安装（保存数据库）</a>
				</div>
				<div class="list-group-item">
					<a href="?do=4" onclick="if(!confirm(\'全新安装将会清空所有数据，是否继续？\')){return false;}" class="btn btn-block btn-warning">全新安装（会删除数据库）</a>
				</div>';
		}else
...
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
	...
	if (!$cn) die('链接数据库失败请检查是否正确:['.mysqli_connect_errno().']'.mysqli_connect_error());
...
<?php
	...
	clearpack();
	echo '<div class="alert alert-info"><font color="green">安装完成！后台管理员账号和密码是:admin/123456</font><br/><br/><a href="../">>>进入7.0首页</a>｜<a href="../admin/">>>进入7.0后台</a><hr/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 install.lock 文件！</font></div>';
?>
	</div>
</div>
...
<div class="progress progress-striped">
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 100%">
	...
	<div class="panel-body">
<?php
	@file_put_contents("install.lock",'已安装锁定，重新安装需要删除本文件！');
	clearpack();
