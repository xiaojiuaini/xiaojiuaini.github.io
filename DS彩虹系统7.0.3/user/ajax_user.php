...
switch ($act) {
    case 'refund':
        ...
        if (time() - strtotime($row['addtime']) > 86400 * 3) {
            exit('{"code":-1,"msg":"该订单超过3天有效期，无法退款，请联系客服处理！"}');
        }
        ...
        if (is_numeric($row['userid'])) {
            ...
            changeUserMoney($userrow['zid'], $money, true, '退款', '订单(ID' . $id . ')已退款到余额');
        }

        rollbackPoint($id);
        $DB->exec("update pre_orders set status='4',result='已经退款' where id='{$id}'");

        exit('{"code":0,"msg":"退款成功，订单资金已退至用户余额，注意查收！"}');
        break;
    case 'setpwd':
    ...
    case 'setClass':
        ...
        break;
    case 'uploadimg':
        if (!$conf['workorder_pic']) {
            exit('{"code":-1,"msg":"未开启上传图片功能"}');
        }
        if ($_POST['do'] == 'upload') {
            $filename = $_FILES['file']['name'];
            $ext      = substr($filename, strripos($filename, '.') + 1);
            $arr      = ['png', 'jpg', 'gif', 'jpeg', 'webp', 'bmp'];
            if (!in_array($ext, $arr)) {
